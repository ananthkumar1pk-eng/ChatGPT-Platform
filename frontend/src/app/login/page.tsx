...
export default function LoginPage() {
  ...
  const handleDemoGoogleLogin = async () => {
    ...
    setError(null);
    try {
      // Simulate Google OAuth ID token for instant test
      const dummyPayload = {
      ...
      // Base64 mock token
      const dummyJwt = `eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.${btoa(JSON.stringify(dummyPayload))}.dummy_signature`;
      await loginWithGoogle(dummyJwt);
    } catch (err: any) {
      ...
      <div className="w-full max-w-md p-8 bg-white dark:bg-[#212121] border border-slate-200 dark:border-slate-800 rounded-3xl shadow-2xl space-y-6 animate-fade-in">
        ...
        <div className="text-center space-y-2">
          ...
          <p className="text-xs text-slate-500 dark:text-slate-400">
            Sign in to access your hosted ChatGPT chats and documents
          </p>
        </div>
        ...
        <button
          ...
          disabled={loading}
          className="w-full py-2.5 px-4 rounded-xl border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 hover:bg-slate-50 dark:hover:bg-slate-800 text-slate-800 dark:text-slate-200 font-semibold text-xs flex items-center justify-center gap-3 transition-all shadow-sm"
        >
        ...
        <div className="relative flex items-center justify-center">
          ...
          <span className="bg-white dark:bg-[#212121] px-3 text-[11px] uppercase font-mono text-slate-400">
            or with email
          </span>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4 text-xs">
          ...
          <div>
            <div className="flex items-center justify-between mb-1.5">
              ...
              <Link href="/reset-password" className="text-emerald-500 hover:underline text-[11px]">
                Forgot password?
              </Link>
            </div>
            <div className="relative">
